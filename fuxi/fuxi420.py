...
class UTransformer(nn.Module):
    ...
    def forward(self, input, temb=None, const=None):
        ...
        if exists(const):
            const = const.to(x)
            if const.ndim == 3:
                const = repeat(const, 'c h w -> n c h w', n=x.size(0))
            elif const.ndim == 4:
                if const.size(0) == 1:
                    const = const.expand(x.size(0), -1, -1, -1)
                elif const.size(0) != x.size(0):
                    raise ValueError(f"const batch 维度不匹配: const.shape={const.shape}, x.shape={x.shape}")
            elif const.ndim == 5:
                const = rearrange(const, 'n t c h w -> (n t) c h w')
                if const.size(0) != x.size(0):
                    raise ValueError(f"const 展平后维度不匹配: const.shape={const.shape}, x.shape={x.shape}")
            else:
                raise ValueError(f"不支持的 const 维度: {const.ndim}, 期望 3/4/5 维")
...
class FuXi(nn.Module):
    ...
    def normalize(self, x, inv=False):
        # 保持全程 torch 张量计算，避免 cuda tensor 与 numpy 混算导致报错
        mean = self.mean.to(device=x.device, dtype=x.dtype)
        std = self.std.to(device=x.device, dtype=x.dtype)

        # 最小清洗：避免输入中的 NaN 传播到后续网络
    ...
    def process_input(self, input, hw=(720, 1440)):
        ...
        # input = self.normalize(input,inv=True)
        input = self.normalize(input)
        print(f'input: {input.shape}, {input.min():.2f} ~ {input.max():.2f}')

        input = input.to(self.dtype)
        const = self.const.to(device=input.device, dtype=self.dtype)
        return input[None], const
    def pre_process(self, x_raw,hw=(720, 1440)):
        # print(f'pre_process: {x_raw.shape}, {x_raw.min():.2f} ~ {x_raw.max():.2f}')
        if (x_raw.shape[-2:] != hw):
            x_raw = F.interpolate(
                x_raw,
                size=hw,
        ...
        # x = x_raw
        tp = x_raw[:, -1:].clamp(min=0, max=1000)
        x_tp = torch.log(1 + tp)
        x_other = x_raw[:, :-1]
        x_1 = torch.cat([x_other, x_tp], dim=1)
        x = (x_1 - self.mean) / self.std
        # const = repeat(self.const,
        #                'c h w -> n t c h w', t=x.size(1), n=x.size(0)
        #                )
    ...
    def post_process(self, x,hw=(721, 1440)):
        if x.shape[-2:] != hw:
            x = F.interpolate(
                x,
                size=hw,
        ...
        x_tp = tp.exp() - 1
        x_other = x[:, :-1]
        x = torch.cat([x_other, x_tp], dim=1)
        return x

    # def post_process(self, x_proc):
    #     # x = x_proc
    #     x_1 = x_proc * self.std + self.mean
    #     tp = x_1[:, :, -1:].clamp(min=0, max=7)
    #     x_tp = tp.exp() - 1
    #     x_other = x_1[:, :, :-1]
    #     x = torch.cat([x_other, x_tp], dim=2)
    #     return x
    ...
    def inference(self, data):
        '''
            hist: b x 2 x 70 x 721 x 1440
            temb: b x 12
            fcst: b x 1 x 70 x 721 x 1440
        '''
        input, temb = data
        input = input.float()

        output = self.decoder[0](input, temb=temb, const=self.const)
        # output = self.decoder[0](input,temb=temb)
        output = rearrange(output, 'n (t c) h w -> n t c h w', t=self.out_frames)
        output = output + input[:, -1:]
        output_temp = output.clone()
        new_input = torch.cat([input, output], dim=1)[:, -self.in_frames:]
        print('inference',output_temp.dtype)
        result = self.post_process(output_temp.float())
        # result = self.post_process(output)

        return new_input, result


    def inference_normal(self, data):
        '''
            hist: b x 2 x 70 x 721 x 1440
            temb: b x 12
            fcst: b x 1 x 70 x 721 x 1440
        '''
        input_data, temb = data
        ...
        input, const = self.pre_process(input_data)

        # input = Variable(input_proc,requires_grad=True)
        decoder = self.decoder[0]
        output = decoder(input, temb=temb, const=const)
        # output = checkpoint(decoder, input, temb, const, use_reentrant=True)
        ...
        new_input = torch.cat([input, output], dim=1)[:, -self.in_frames:]
        # print('inference', output_temp.dtype)
        output_temp=rearrange(output_temp, 'n t c h w -> (n t) c h w')
        new_input=rearrange(new_input, 'n t c h w -> (n t) c h w')


        # print('output_temp.shape', output_temp.shape)
        # print('new_input.shape', new_input.shape)

        inv_result = self.post_process(output_temp.float())
        # inv_result=inv_result.squeeze(0)

        # print('inv_result.shape', inv_result.shape)

        inv_new_input = self.post_process(new_input.float())
        # print('inv_new_input.shape', inv_new_input.shape)
        # res_grad = self.grad_process(grad, input_proc)

        return inv_new_input, inv_result


    def compute_grad(self, J,input):
        # if not self.save_dir.endswith("grad"):
        #     return
        print('J',J)
        ...
        grad = torch.autograd.grad(outputs=[J], inputs=[input], retain_graph=True)[0]
        # print(grad)
        # print(grad.shape)
        # print(init_time)    777u6

        return grad

    def J(self,area_indice,pred,truth,indice,weighting=None):
        # lon_round=(4*area[0],4*area[1])
        print('test')
        print(pred.shape)
        # print(truth.shape)
        print([indice_tmp for indice_tmp in indice])

        pred_tmp=pred[:,:,[indice_tmp for indice_tmp in indice],area_indice[3]:area_indice[2],area_indice[0]:area_indice[1]]
        truth_tmp=truth[:,[indice_tmp for indice_tmp in indice],area_indice[3]:area_indice[2],area_indice[0]:area_indice[1]].unsqueeze_(0)
        if weighting == None:
            J = ((pred_tmp-truth_tmp)**2).mean()
            # J = ((pred_tmp - truth_tmp) ** 2).sum()
        else:
            weighting=weighting.view(1,1,1,-1,1)
            print(((pred_tmp-truth_tmp)*weighting).shape)
            J=(((pred_tmp-truth_tmp)*weighting)**2).mean()
            # J = (((pred_tmp - truth_tmp) * weighting) ** 2).sum()
        return J
