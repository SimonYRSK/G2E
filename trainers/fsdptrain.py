...
class FSDPTrainer(BaseTrainer):
    def __init__(
        ...
        **kwargs
        ...
        self.global_step = 0
        self.use_fsdp = use_fsdp and torch.cuda.is_available() and self.world_size > 1
    ...
    def _setup_fsdp(self):
        ...
        if self.rank == 0:
            print(f"✅ FSDP 已启用，使用 {self.world_size} 张 GPU")
    ...
    def _destroy_fsdp(self):
        """清理 FSDP 资源"""
        if hasattr(self.model, "module"):
            from torch.distributed.fsdp import FullyShardedDataParallel as FSDP
            FSDP.summon_full_params(self.model)
