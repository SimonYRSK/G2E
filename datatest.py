...
gfs2era5_mapping = {
    "Temperature": {
        "era5_vars": ["t50", "t100", "t150", "t200", "t250", "t300",
    ...
    "U component of wind": {
        "era5_vars": ["u50", "u100", "u150", "u200", "u250", "u300",
                      "u400", "u500", "u600", "u700", "u850", "u925", "u1000"],
        "var_type": "upper_air",
        "description": "GFS通用U风分量 → ERA5 50~1000hPa等压面纬向风（13层）"
    },
    "V component of wind": {
        "era5_vars": ["v50", "v100", "v150", "v200", "v250", "v300",
                      "v400", "v500", "v600", "v700", "v850", "v925", "v1000"],
        "var_type": "upper_air",
        "description": "GFS通用V风分量 → ERA5 50~1000hPa等压面经向风（13层）"
    },
    "Geopotential height": {
        "era5_vars": ["z50", "z100", "z150", "z200", "z250", "z300",
                      "z400", "z500", "z600", "z700", "z850", "z925", "z1000"],
        "var_type": "upper_air",
...
class ERA5Reader:
    """直接读取 ERA5 zarr（data/channel/lat/lon），支持按 GFS 通用变量名取 13 层 / 地面变量。"""
    ...
    def __init__(
        ...
        self.data_zarr: Optional[zarr.Array] = None
        self.channel_zarr = None
        self.lat_zarr = None
        self.lon_zarr = None
        self.valid_time_indices: Optional[List[int]] = None
        self.channel_name2idx: Optional[Dict[str, int]] = None

        self._load_zarr_handles()
        self.valid_timestamps = self._generate_valid_timestamps()

        # 预先构建 gfs_var -> era5 通道索引数组，避免 read_by_time 里频繁查字典
        self.gfs2era5_chanidx: Dict[str, np.ndarray] = {}
        for gfs_var, era5_vars in self.gfs2era5_vars.items():
            idxs: List[int] = []
            for ev in era5_vars:
                if ev not in self.channel_name2idx:
                    raise ValueError(f"ERA5无该通道：{ev}")
                idxs.append(self.channel_name2idx[ev])
            self.gfs2era5_chanidx[gfs_var] = np.asarray(idxs, dtype="int64")
    ...
    def _time_filter(self) -> List[int]:
        ...
        if start_idx > end_idx:
            raise ValueError(
                f"ERA5无有效时间数据：{self.start_dt}~{self.end_dt}（索引{start_idx}~{end_idx}超出范围）"
            )
    ...
    def _get_nearest_time_idx(self, target_time: datetime) -> int:
        ...
        valid_indices_arr = np.asarray(self.valid_time_indices, dtype="int64")
        nearest_idx = int(valid_indices_arr[np.argmin(np.abs(valid_indices_arr - target_idx))])
    ...
    def read_by_time(
        ...
        for gfs_var in gfs_vars:
            if gfs_var not in self.gfs2era5_chanidx:
                raise ValueError(f"ERA5不支持GFS通用变量：{gfs_var}")

            idx_arr = self.gfs2era5_chanidx[gfs_var]  # (L,)

            # 关键：一次性取出所有层 (L, H, W)，利用 zarr 的块读
            var_data_3d = self.data_zarr.oindex[
                (time_idx, idx_arr, slice(None), slice(None))
            ]

            result[gfs_var] = np.nan_to_num(var_data_3d, nan=0.0)
...
class GFSReader:
    """通过 xarray 从 GFS Zarr 读取，支持 upper_air / surface 变量。"""
    ...
    def __init__(
        ...
        self.valid_time_indices: Optional[List[int]] = None
        self.valid_time_stamps: Optional[List[datetime]] = None
        self.level_order = [50, 100, 150, 200, 250, 300,
                            400, 500, 600, 700, 850, 925, 1000]

        self.full_time_index: Optional[pd.DatetimeIndex] = None
        self.level_vals: Optional[List[int]] = None
        self.lat_len: Optional[int] = None
        self.lon_len: Optional[int] = None

        # 懒加载的 xarray.Dataset 句柄（每个 worker 进程一份）
    ...
    def _load_metadata(self):
        """只用一次 open_zarr 读取元数据。"""
        try:
            with xr.open_zarr(self.gfs_root, consolidated=True) as ds:
                ...
                self.full_time_index = pd.DatetimeIndex(
                    ds["time"].values.astype("datetime64[s]")
                )
                self.level_vals = ds["level"].values.tolist()
    ...
    def _ensure_dataset(self):
        """懒加载持久 ds；在 DataLoader 多进程下，每个 worker 第一次用时各开一次。"""
        if self.ds is None:
            self.ds = xr.open_zarr(self.gfs_root, consolidated=True)
    ...
    def _get_nearest_time_idx(self, target_time: datetime) -> Tuple[int, datetime]:
        ...
        nearest_pos = int(np.argmin(time_diffs))
    ...
    def read_by_time(
        ...
        ds_time = self.ds.isel(time=time_idx)
        ...
        for gfs_var in gfs_vars:
            ...
            if var_type == "upper_air":
                ...
                var_arr = var_data.isel(level=level_indices).values  # (13,H,W)
            else:
                var_arr = var_data.values[np.newaxis, :, :]          # (1,H,W)
...
def _default_norm_cache_path(gfs_reader, gfs_vars: List[str], base_layers: int, pad_mode: str) -> Path:
    ...
    cache_dir = Path(
        "/cpfs01/projects-HDD/cfff-4a8d9af84f66_HDD/public/database/gfs_2020_2024_c10"
    )
...
class GFSERA5PairDataset(Dataset):
    ...
    def __init__(
        ...
        # 构建展开后的通道列表
        self.flat_var_names: List[str] = []
        self.var_info: List[Dict] = []

        level_suffixes = [str(l) for l in self.gfs_reader.level_order]
        ...
        if self.normalize:
            ...
            if cache_path.exists():
                ...
                print(f"⚠️ 未找到归一化文件: {cache_path}，将不执行标准化")
    ...
    def _get_flattened_data(self, reader, ts, is_gfs: bool = True) -> np.ndarray:
        """读取一次时间步的所有变量并展开到 C 维。"""
        raw_dict = reader.read_by_time(ts, self.raw_gfs_vars, verbose=False)

        extracted_channels: List[np.ndarray] = []
        ...
        return np.concatenate(extracted_channels, axis=0)  # (C,H,W)
    ...
    def __getitem__(self, idx):
        if self.temporal_pair:
            ...
            g_pair = np.stack([g_prev, g_curr], axis=1)  # (C,2,H,W)
            e_pair = np.stack([e_prev, e_curr], axis=1)

            return (
                torch.from_numpy(g_pair).float(),
                torch.from_numpy(e_pair).float(),
                (
                    ts_prev.strftime("%Y-%m-%d %H:%M:%S"),
                    ts_curr.strftime("%Y-%m-%d %H:%M:%S"),
                ),
            )
        else:
            ts = self.common_timestamps[idx]
            g_data = self._get_flattened_data(self.gfs_reader, ts, is_gfs=True)
            e_data = self._get_flattened_data(self.era5_reader, ts, is_gfs=False)

            return (
                torch.from_numpy(g_data).float(),
                torch.from_numpy(e_data).float(),
